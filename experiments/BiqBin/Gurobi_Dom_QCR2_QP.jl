...
function run(fname)
    ...
    if isfile(logname)
        ...
        one_solve(N, W, logname, qcr_cut = true , cut=false, grb_solver=true, QCR2=true , root=true )
        one_solve(N, W, logname, qcr_cut = true , cut=false, grb_solver=true, QCR2=true , root=false )
    end
