...
function run(fname)
    ...
    if isfile(logname)
        ...
        one_solve(N, W, logname, qcr_cut = true , cut=true, grb_solver=true, QCR=true , root=true )
        one_solve(N, W, logname, qcr_cut = true , cut=true, grb_solver=true, QCR=true , root=false )
    end
