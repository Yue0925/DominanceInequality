...
function run(fname)
    ...
    folder = "./res/Cplex/Dom_QCR2_QP_Dom"
    if !isdir(folder)
        mkdir(folder)
    end
    logname = folder * "/" *split(fname, "/")[end]
    if isfile(logname)
        ...
        println("loging $logname ... ")
        one_solve(N, W, logname, qcr_cut = true, cut=true, grb_solver=false, QCR2=true , root=true )
        one_solve(N, W, logname, qcr_cut = true, cut=true, grb_solver=false, QCR2=true , root=false )
    end 
