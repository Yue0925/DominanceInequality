...
function run(fname)
    ...
    if isfile(logname)
        ...
        println("loging $logname ... ")
        one_solve(N, W, logname, cut=false, grb_solver=false, QCR2=true , root=true )
        one_solve(N, W, logname, cut=false, grb_solver=false, QCR2=true , root=false )
    end
