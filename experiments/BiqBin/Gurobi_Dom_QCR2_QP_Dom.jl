...
function run(fname)
    ...
    folder = "./res/Gurobi/Dom_QCR2_QP_Dom"
    if !isdir(folder)
        mkdir(folder)
    end
    ...
    if isfile(logname)
        ...
        one_solve(N, W, logname, qcr_cut = true , cut=true, grb_solver=true, QCR2=true , root=true )
        one_solve(N, W, logname, qcr_cut = true , cut=true, grb_solver=true, QCR2=true , root=false )
    end
